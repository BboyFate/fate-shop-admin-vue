  ...
  methods: {
    ...
    handleTemplateToAttr() {
      ...
      this.productAttrTemplates[this.currentProductAttrTemplateIndex].attributes.forEach(newProductAttr => {
        ...
        // 旧规格里有模板的里的规格
        if (oldProductAttr) {
          let newProductAttrValues = []
          // 取出有模板里的规格值的旧规格值
          newProductAttr.values.forEach(newValue => {
            let item = oldProductAttr.values.find(oldValue => oldValue.value === newValue)
            if (item) {
          ...
          // 现在的规格里不存在 模板的规格
          let newProductAttrValues = newProductAttr.values.map(newValue => ({
            id: generateUniqueString(newValue),
            value: newValue
          }))
